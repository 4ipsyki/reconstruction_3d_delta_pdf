function [dk,P] = calc_dk_P(orgx,orgy,det_dist,ki_vec,rot_ax,...
...
% change here for Pilatus detector
% include detector rotation here
...
det_rot_z=[det_x; det_y; 0,0,1]; % orientation: here for Pilatus [010;100;001]
%det_rot_z=[1,0,0; 0,-1,0; 0,0,1]; % orientation: here for PE/P07 [100;0-10;001]
%
% [kx;ky;kz]=det_rot_x * det_rot_y * det_rot_z * [(n-orgy)*dy; (m-orgx)*dx; zeros(x_size,y_size)+det_dist];
%%% big change here
det_matrix(:,:,1)=(m-orgx)*dx;
det_matrix(:,:,2)=(n-orgy)*dy;
%det_matrix(:,:,3)=zeros(x_size,y_size)+det_dist;  % this is for rotation around sample position (see below)
det_matrix(:,:,3)=zeros(x_size,y_size);            % this is for rotation around beam position on detector (see below)
...
kx=kaa_x_rotxy;
ky=kaa_y_rotxy;
%kz=kaa_z_rotxy            % this is for rotation around sample position (see above)
kz=kaa_z_rotxy+det_dist;   % this is for rotation around beam position on detector (see above)
%ky=-(n-orgy)*dy;
%kx= (m-orgx)*dx;
%kz=zeros(x_size,y_size)+det_dist;
...
% L=|S dot (u x S0)| / (|S| dot |S0|) this is wrong!!!! see:

% Kabsch Acta Crystallogr D Biol Crystallogr. 2014, 70 2204–2216: 
% L=|u dot (S x S0)| / (|S| dot |S0|)  which is the same!!
...
%cross_rot_ax_ki(:,:,1) = rot_ax(2).*ki(:,:,3) - rot_ax(3).*ki(:,:,2);
%cross_rot_ax_ki(:,:,2) = rot_ax(3).*ki(:,:,1) - rot_ax(1).*ki(:,:,3);
%cross_rot_ax_ki(:,:,3) = rot_ax(1).*ki(:,:,2) - rot_ax(2).*ki(:,:,1);
cross_kf_ki(:,:,1) = kf(:,:,2).*ki(:,:,3) - kf(:,:,3).*ki(:,:,2);
cross_kf_ki(:,:,2) = kf(:,:,3).*ki(:,:,1) - kf(:,:,1).*ki(:,:,3);
cross_kf_ki(:,:,3) = kf(:,:,1).*ki(:,:,2) - kf(:,:,2).*ki(:,:,1);

%dot_kf_cross_rot_ax_ki=kf(:,:,1).*cross_rot_ax_ki(:,:,1) + kf(:,:,2).*cross_rot_ax_ki(:,:,2) + kf(:,:,3).*cross_rot_ax_ki(:,:,3);
